...
int main() {
      ...
      matrizInit( &m1, *m, l, c );
      preencherMatriz( &m1 );
      printf("matriz antes de trocada por seu mod\n");
      mostrarMatriz( m1 );
      printf("matriz depois de trocada por seu mod\n");
      matrizMod(&m1);
      mostrarMatriz( m1 );
      return 0;
}
...
void matrizMod( matriz *container ) {
      for (int i = 0; i < container -> l; i++) {
            for (int j = 0; j < container -> c; j++) {
                  if (container -> m[calcularIndex(i, j, container -> c)] < 0) {
                        container -> m[calcularIndex(i, j, container -> c)] *= -1;
                  }
            }
      }
}
